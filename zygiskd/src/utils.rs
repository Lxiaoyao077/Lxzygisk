...
/// Non-fatal: on devices without SELinux (or when running outside Android,
/// e.g. host tests) the call simply has no effect.
pub fn set_socket_create_context(context: &str) -> Result<()> {
    ...
    if let Err(e) = fs::write(&fallback_path, context) {
        log::warn!("set_socket_create_context({context}) failed (non-fatal): {e}");
    }
    Ok(())
}
...
pub fn unix_listener_from_path(path: &str) -> Result<UnixListener> {
    ...
    // chcon only matters on Android; on other platforms (host tests, manual
    // daemon startup) it is unavailable and must not be fatal.
    if let Err(e) = chcon(path, "u:object_r:zygisk_file:s0") {
        log::warn!("chcon({path}) failed (non-fatal): {e}");
    }
