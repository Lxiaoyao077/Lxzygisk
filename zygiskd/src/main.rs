    ...
    match args.get(1).map(String::as_str) {
        ...
        _ => {
            ...
            if let Err(e) = main_daemon_entry(workdir) {
                ...
                // Also surface on stderr: invisible on Android (logcat is the
                // sink there) but crucial when starting the daemon manually
                // on a host or from adb shell.
                eprintln!("zygiskd: daemon failed: {:?}", e);
            }
        }
    }
}
