...
// management surface (e.g. a shell frontend) can drive them directly.

/// The parsed APatch package configuration (used by the WebUI denylist page).
#[allow(dead_code)]
pub fn apatch_config() -> Vec<apatch::PackageConfig> {
    apatch::read_package_config()
}

/// Updates the allow/exclude flags of one APatch package entry.
#[allow(dead_code)]
pub fn apatch_set(
    pkg: &str,
    uid: i32,
    ...
    apatch::set_package_flags(pkg, uid, allow, exclude)
}

/// Removes an APatch package entry.
#[allow(dead_code)]
pub fn apatch_remove(pkg: &str, uid: Option<i32>) -> anyhow::Result<()> {
    apatch::remove_package(pkg, uid)
}

/// The APatch daemon version code, if apd is installed.
#[allow(dead_code)]
